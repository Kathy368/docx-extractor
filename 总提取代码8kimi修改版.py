...
更新说明（v8）：
- 导入模块版本同步为 extract_report_test2_docx3中文成功plus5 / extract_report_test2_docx2英文成功5
- 中文/英文报告提取均接入《检测要求和检测结论字段专属提取方案2》的字段逻辑，
  并保留原有逻辑作为兜底，确保检测要求/检测结论字段提取更完善且不降低已有效果。
- 扫描阶段新增第一页内容校验：文件名像报告但第一页含申请单/委托单关键词的文件，
  会被排除并单独记录在汇总 Excel 的「第一页内容排除文件」Sheet 中。
"""
...
# ====== 1. 路径配置 ======
# 报告根目录路径
ROOT_FOLDER = r"G:\中心实验室\中心实验室-新(2023.07.31)\002 报告\报告组\常规组报告 2017.11月份开始\待审核报告 新\昆山（报告布局居中）"
...
# 导入中文/英文提取模块
# ==============================================
import extract_report_test2_docx3中文成功plus5kimi修改版 as _zh_module
import extract_report_test2_docx2英文成功5kimi修改版 as _en_module

# 同步导入模块的 VERBOSE 开关，避免批量时产生过多日志
_zh_module.VERBOSE = DEBUG_MODE
_en_module.VERBOSE = DEBUG_MODE

from extract_report_test2_docx3中文成功plus5kimi修改版 import (
    fix_docx_if_needed,
    get_cell_text,
    normalize_text,
    normalize_key,
...
# 第一页申请表/委托单关键词：文件名像报告，但第一页含这些词 → 排除
FIRST_PAGE_FORM_KEYWORDS = [
    '申请单', '检测申请单', '测试申请单',
    '委托单', '委托检测申请单', '检测委托单',
    '测试委托单', '委托测试申请单',
]
...
def is_application_form_by_first_page(doc):
    """
    检查文件第一页是否含有申请表/委托单类关键词。
    扫描范围：前 10 个段落 + 前 3 个表格的表头/首行。
    若命中，说明该文件虽文件名像报告，实际为申请单/委托单，应排除。
    """
    # 检查前 10 个段落
    for p in doc.paragraphs[:10]:
        text = normalize_text(p.text)
        if any(kw in text for kw in FIRST_PAGE_FORM_KEYWORDS):
            return True
    ...
    for table in doc.tables[:3]:
        ...
        if any(kw in row_text for kw in FIRST_PAGE_FORM_KEYWORDS):
            return True

    return False


def check_report_file(file_path, project_name, report_files_list, content_excluded_list):
    """
    综合文件名 + 第一页内容判断是否为报告。
    文件名判定通过后再打开文档检查第一页；若第一页含申请单/委托单关键词，则排除。
    文档打不开时，退回到文件名判定。
    """
    filename = os.path.basename(file_path)
    if not is_report_by_filename(filename):
        return

    try:
        actual_file = fix_docx_if_needed(file_path)
        doc = Document(actual_file)
        if is_application_form_by_first_page(doc):
            content_excluded_list.append({
                "文件路径": file_path,
                "文件名": filename,
                "所在项目": project_name,
                "情况说明": "文件名像报告，但第一页含申请单/委托单关键词"
            })
            return
    except Exception:
        # 文档损坏/无法打开时，按文件名兜底保留
    ...
    report_files_list.append(file_path)
...
def scan_all_folders(root_folder):
    ...
    report_files = []
    no_report_projects = []
    content_excluded_files = []  # 文件名像报告，但第一页为申请表/委托单
    ...
    if root_docx:
        root_reports = []
        for f in root_docx:
            check_report_file(f, "【根目录】", root_reports, content_excluded_files)
        ...
        if root_reports:
            ...
            print(f"   [根目录] 发现 {len(root_docx)} 个 DOCX，其中 {len(root_reports)} 份报告"
                  f"（内容排除 {len([e for e in content_excluded_files if e['所在项目'] == '【根目录】'])} 份）")
        else:
        ...
        for dirpath, dirnames, filenames in os.walk(project_path):
            ...
            for f in filenames:
                ...
                if f_lower.endswith('.docx'):
                    ...
                    check_report_file(f_path, project, project_reports, content_excluded_files)
    ...
    print(f"   识别报告：{len(report_files)} 份 | 第一页内容排除：{len(content_excluded_files)} 份 | 待确认项目：{len(no_report_projects)} 个")

    return report_files, no_report_projects, content_excluded_files
...
from extract_report_test2_docx3中文成功plus5kimi修改版 import (
    ...
    extract_dates_from_paragraphs_zh,
    find_column_index,
    ZH_FIELD_MAPPING,
    extract_requirement_zh,
    extract_conclusion_zh,
)
...
def extract_chinese_report(doc, filename):
    ...
    if not req or not con:
        old_req, old_con = extract_test_req_conclusion_zh(doc)
    ...
    if direct_methods:
        ...
        if not test_methods and doc_type == "long":
        ...
        if test_methods and paragraph_methods:
        ...
        elif not test_methods and paragraph_methods:
...
from extract_report_test2_docx2英文成功5kimi修改版 import (
    extract_english_report as extract_english_report_module,
    extract_requirement_en,
    extract_conclusion_en,
    extract_dates_from_paragraphs_en,
)
...
def extract_english_report(doc, filename, file_path=None):
    """
    英文报告完整提取
    直接复用英文成功5模块的 extract_english_report，以获得：
    - 标准格式表格提取
    - 无序号样品统计兜底
    - 图片型报告的 OCR 识别兜底
    返回前补充/修正以下字段：
    ...
    - 检测结论：使用专题方案2英文结论提取逻辑；仍无则提示需人工查看
    """
    result, _ = extract_english_report_module(doc, filename, file_path)

    # 1. 样品部件总数：直接使用英文成功5模块的结果（已包含 Test Result(s) / Sample Description 多层兜底）
    ...
    # 3. 检测要求/结论：使用专题方案2逻辑补充/覆盖（模块内部已调用，这里做二次确认）
    if not result.get("检测要求", "").strip():
...
def main():
    ...
    report_files, no_report_projects, content_excluded_files = scan_all_folders(ROOT_FOLDER)
    ...
    if all_results:
        ...
        # 第一页内容排除文件清单
        df_content_excluded = pd.DataFrame(content_excluded_files) if content_excluded_files else pd.DataFrame(
            columns=["文件路径", "文件名", "所在项目", "情况说明"])
        ...
        with pd.ExcelWriter(merged_excel, engine='openpyxl') as writer:
            ...
            if not df_content_excluded.empty:
                df_content_excluded.to_excel(writer, sheet_name='第一页内容排除文件', index=False)

        sheet_count = 3 + (1 if not df_content_excluded.empty else 0)
        print(f"  [OK] 汇总版：{len(all_results)} 份 → {os.path.basename(merged_excel)}")
        print(f"     含 {sheet_count} 个 Sheet：提取结果汇总 / 待人工确认项目 / 提取异常清单"
              f"{' / 第一页内容排除文件' if not df_content_excluded.empty else ''}")
    ...
    print(f"   成功：{success} 份 | 失败：{error_count} 份 | 第一页内容排除：{len(content_excluded_files)} 份")
